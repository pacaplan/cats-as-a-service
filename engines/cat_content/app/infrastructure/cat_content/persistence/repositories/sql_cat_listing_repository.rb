    ...
    module Persistence
      module Repositories
        class SqlCatListingRepository < Ports::CatListingRepository
          ...
          def find(id)
            rec = CatContent::CatListingRecord.find_by(id: id.to_s)
            rec && @mapper.to_domain(rec)
          end

          def find_by_slug(slug)
            rec = CatContent::CatListingRecord.find_by(slug: slug.to_s, visibility: "public")
            rec && @mapper.to_domain(rec)
          end

          def list_all(filters: {}, page: 1, per_page: 20)
            scope = CatContent::CatListingRecord.all
          ...
          def list_public(tags: [], page: 1, per_page: 20)
            scope = CatContent::CatListingRecord.where(visibility: "public")
          ...
          def remove(id)
            CatContent::CatListingRecord.where(id: id.to_s).delete_all
          end
        end
      end
    end
